...
#endif
const int max_idle_time = 10000; //10秒
...
void game_room::on_round_begin(userid uuid, const Json::Value &data)
{
    ...
    if (!is_all_ready){ //不是所有人都准备好了
        send((protocol)niuniu_round_begin, notify, uuid, 2);
        return;
    }
    if (count_ready < 2){ //至少要有2个人准备好了才能开局
        send((protocol)niuniu_round_begin, notify, uuid, 3);
        return;
    }
...
void game_room::init_user_poker()
{
    ...
    for (int i = 0; i < index; i++){
        ...
        std::random_shuffle(i_array.begin(), i_array.end());
        int j = i_array[rand() % (int)i_array.size()];
        TRACE("User %d select poker %d.\r\n", i, unused[j]);
