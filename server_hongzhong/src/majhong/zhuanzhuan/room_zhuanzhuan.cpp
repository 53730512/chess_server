...
namespace zhuanzhuan_mj{
	...
	bool game_room::init_room_rules(const Json::Value &option)
	{
		...
		{
			m_members.push_back(new zhuanzhuan_player());
		}
		...
		m_zhuangxian = option["zhuangxian"].asBool();
		m_enable_chi = false;
		return true;
	}
	...
	void game_room::publish_result(int winner, bool completed)
	{
		...
		if(!winner_list.empty())
		{
			...
			{
				//庄额外加1分
				if(m_zhuang == winner_list[0]){	
					if(m_zhuangxian)
						m_members[m_zhuang]->round_score += 3*(m_members.size() -1);
					else
						m_members[m_zhuang]->round_score += 2*(m_members.size() -1);

					//鸟
					m_members[m_zhuang]->round_score += m_members[m_zhuang]->zhongniao*(m_members.size() -1);
					for(size_t i=0; i<m_members.size(); i++)
					{
						if(i== m_zhuang)
							continue;
						...
						if(m_zhuangxian)
							m_members[i]->round_score -= 3;
						else
							m_members[i]->round_score -= 2;
						m_members[i]->round_score -= m_members[m_zhuang]->zhongniao;
					}
				}
				else
				{
					...
					if(m_zhuangxian)
						m_members[winner_list[0]]->round_score++;

					//鸟
					m_members[winner_list[0]]->round_score += m_members[winner_list[0]]->zhongniao*(m_members.size() -1);
					for(size_t i=0; i<m_members.size(); i++)
					{
						if(i== winner_list[0])
							continue;
						...
						m_members[i]->round_score -= 2;
						if(i==m_zhuang && m_zhuangxian)
							m_members[i]->round_score--;

						m_members[i]->round_score -= m_members[winner_list[0]]->zhongniao;
					}
				}
				...
				for(size_t i=0; i<winner_list.size(); i++)
				{
					...
					player->round_score += player->zhongniao;
					if(m_zhuangxian)
					{
						//庄放炮,额外得1分
						if(m_cur_player == m_zhuang || winner_list[i] == m_zhuang)
							player->round_score++;
					}
				}

				m_members[m_cur_player]->round_score -= winner_list.size();
				m_members[m_cur_player]->round_score -= winner_list.size()*m_members[winner_list[0]]->zhongniao;
				if(m_zhuangxian)
				{
					if(m_cur_player == m_zhuang)
						m_members[m_cur_player]->round_score -= winner_list.size();
					else
					{
						//如果胡牌人中有一个是庄,则多出1分
						if(std::find(winner_list.begin(), winner_list.end(), m_zhuang) != winner_list.end())
							m_members[m_cur_player]->round_score--;
					}
				}
			}
		}
		...
		if(m_zhuaniao > 0){
			for(int i=0; i<m_zhuaniao; i++){
				if(get_left_card_number() <= 0){
					if(!m_niao.empty()){
						m_niao.push_back(m_niao[m_niao.size() - 1]);
					}
					else{
						m_niao.push_back(m_cards_end_id);
					}
				}
				else{
	...
	bool game_room::check_guo_hu(int index)
	{
